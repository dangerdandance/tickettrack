...
export default function TicketTracker() {
  ...
  // Load tickets from Sheet on mount
  useEffect(() => {
    (async () => {
  ...
  const processFile = async (file) => {
    ...
    try {
      ...
      if (result && !result.error) {
        setCurrentTicket({ ...result, _file: file, _b64: b64, _mime: file.type, _id: Date.now(), _confirmed: false });
      } else {
  ...
  const weekStats  = sumByCat(weekTickets);
  const totalMonth = monthStats.reduce((s,c) => s + c.spent, 0);
  const totalWeek  = weekStats.reduce((s,c) => s + c.spent, 0);
          ...
          <div>
            ...
            {!currentTicket && !processing && (
              <div
                ...
                style={{
                  border:`2px dashed ${dragging ? "#60a5fa" : "#2a3a4f"}`,
                  ...
                  <div style={{ background:"#0d1420", borderRadius:10, padding:12, marginBottom:16 }}>
                    ...
                    {currentTicket.items.map((item, i) => (
                      <div key={i} style={{ display:"flex", justifyContent:"space-between", fontSize:13, padding:"4px 0", borderBottom: i < currentTicket.items.length-1 ? "1px solid #1a2535" : "none" }}>
                        ...
                        <span style={{ color:"#60a5fa" }}>{fmt(item.precio)}</span>
                      </div>
                    ))}
                  </div>
                )}
        ...
        {tab === "dashboard" && (
          <div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:20 }}>
              {[
                { label:"Gasto este mes",  value:fmt(totalMonth), sub:`de ${fmt(MONTHLY_BUDGET)}`,
                  color: totalMonth > MONTHLY_BUDGET ? "#f87171" : "#4ade80" },
                { label:"Meta de ahorro",  value:fmt(SAVINGS_GOAL),
                  sub: totalMonth <= MONTHLY_BUDGET - SAVINGS_GOAL ? "✅ En camino" : "⚠️ Revisa gastos",
                  color: totalMonth <= MONTHLY_BUDGET - SAVINGS_GOAL ? "#4ade80" : "#fbbf24" },
            ...
            <div style={{ background:"#111827", borderRadius:14, padding:16, marginBottom:16, border:"1px solid #1e2a3a" }}>
              ...
              <div style={{ background:"#1e2a3a", borderRadius:6, height:8, overflow:"hidden" }}>
                <div style={{
                  ...
                  width:`${Math.min((totalMonth/MONTHLY_BUDGET)*100,100)}%`,
                  background: totalMonth > MONTHLY_BUDGET ? "#f87171" : totalMonth > MONTHLY_BUDGET*0.8 ? "#fbbf24" : "#4ade80",
